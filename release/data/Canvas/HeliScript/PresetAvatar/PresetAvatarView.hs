component PresetAvatarView
{
    ...
    void _CreatePresetAvatarList(bool IsPortrait, int PresetAvatarCount, int MaxColCount, int XOffset, int YOffset)
    {
        ...
        for(int AvatarID = 0; AvatarID < PresetAvatarCount; AvatarID++)
        {
            ...
            if(system.IsLangJA())
            {
                ThumbnailURI = hsPlayerGetPresetAvatarThumbnailURL(AvatarID);
            }
            else
            {
                ThumbnailURI = hsPlayerGetPresetAvatarEN_ThumbnailURL(AvatarID);
            }
            ...
            if(IsPortrait)
            {
                hsAddGUIButton(
                    ...
                    ThumbnailURI, "", "", "", 280, 280, 300, 32, 0, 0, 0, 0, 0, 0, 0, 0
                );

                hsAddGUIImage(
                    layerName, IsPortrait,
                    PresetAvatarSelectmark, "", "", "", true, 8, -330 + row * XOffset, -320 + col * YOffset, 100, 100, 0.5, 0.5, "CM", true, 0.0,
                    "gui2023/menu/mypage/mypage_select_icon.png", 0, 0, 0, 0, 0, 0, 0, 0
                );
            }
            else
            {
                hsAddGUIButton(
                    ...
                    ThumbnailURI, "", "", "", 120, 120, 300, 32, 0, 0, 0, 0, 0, 0, 0, 0
                );
